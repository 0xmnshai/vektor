...
GPUTexture *GPU_texture_create_2d(int width, int height, eGPUTextureFormat format)
{
  ...
  if (creator::G.gpu_backend == creator::GPU_BACKEND_OPENGL) {
    ...
    MTLTextureDescriptor *texDesc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:(format == GPU_DEPTH_COMPONENT24 ? MTLPixelFormatDepth32Float : MTLPixelFormatRGBA8Unorm)
                                                                                       width:width
                                                                                      height:height
                                                                                   mipmapped:NO];
