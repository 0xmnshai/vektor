...
template <typename T>
struct ListBaseEnumerateIterator
{
public:
...
public:
    ListBaseEnumerateIterator(T*  data,
                              int index)
        : data_(data)
        , index_(index)
    {
    ...
    friend bool operator==(const ListBaseEnumerateIterator& a,
                           const ListBaseEnumerateIterator& b)
    {
        return a.data_ == b.data_;
    }
    ...
    value_type operator*() const { return {index_, *data_}; }
};
...
struct ListBaseEnumerateWrapper
{
    ...
    ListBaseEnumerateIterator<T> begin() const { return ListBaseEnumerateIterator<T>(static_cast<T*>(first), 0); }

    ListBaseEnumerateIterator<T> end() const { return ListBaseEnumerateIterator<T>(nullptr, 0); }
};

/** An iterator for use with #ListBase that caches the next pointer to allow removal. */
template <typename T>
struct ListBaseMutableIterator
{
public:
    using iterator_category = std::forward_iterator_tag;
    using value_type        = T;
...
public:
    ListBaseMutableIterator(T* data)
        : current_(data)
    {
        if (current_)
        {
    ...
    friend bool operator==(const ListBaseMutableIterator& a,
                           const ListBaseMutableIterator& b)
    {
...
struct ListBaseMutableWrapper
{
    ...
    ListBaseMutableIterator<T> begin() const { return ListBaseMutableIterator<T>(static_cast<T*>(first)); }

    ListBaseMutableIterator<T> end() const { return ListBaseMutableIterator<T>(nullptr); }
};

/** An iterator for use with #ListBase iterating backwards. */
template <typename T>
struct ListBaseBackwardIterator
{
public:
    using iterator_category = std::forward_iterator_tag;
    using value_type        = T;
...
public:
    ListBaseBackwardIterator(T* data)
        : data_(data)
    {
    }
    ...
    friend bool operator==(const ListBaseBackwardIterator& a,
                           const ListBaseBackwardIterator& b)
    {
        return a.data_ == b.data_;
    }
...
struct ListBaseBackwardWrapper
{
    ...
    ListBaseBackwardIterator<T> begin() const { return ListBaseBackwardIterator<T>(static_cast<T*>(last)); }

    ListBaseBackwardIterator<T> end() const { return ListBaseBackwardIterator<T>(nullptr); }
};

/** An iterator for use with #ListBase iterating backwards and allowing removal. */
template <typename T>
struct ListBaseMutableBackwardIterator
{
public:
    using iterator_category = std::forward_iterator_tag;
    using value_type        = T;
...
public:
    ListBaseMutableBackwardIterator(T* data)
        : current_(data)
    {
        if (current_)
        {
...
struct ListBaseMutableBackwardWrapper
{
    ...
    ListBaseMutableBackwardIterator<T> end() const { return ListBaseMutableBackwardIterator<T>(nullptr); }
};
