...
struct IDHash {
  ...
  friend bool operator!=(const IDHash &a, const IDHash &b) { return !(a == b); }
...
#define ID_FAKE_USERS(id)                                                      \
  ((id_cast<const ID *>(id)->flag & ID_FLAG_FAKEUSER) ? 1 : 0)

#define ID_EXTRA_USERS(id)                                                     \
  (id_cast<const ID *>(id)->tag & ID_TAG_EXTRAUSER ? 1 : 0)

#define ID_EXTRA_REAL_USERS(id)                                                \
  (id_cast<const ID *>(id)->tag & ID_TAG_EXTRAUSER_SET ? 1 : 0)
...
#define ID_IS_PACKED(_id)                                                      \
  (ID_IS_LINKED(_id) && ((_id)->flag & ID_FLAG_LINKED_AND_PACKED))
...
#define ID_IS_OVERRIDABLE_LIBRARY_HIERARCHY(_id)                               \
  (ID_IS_LINKED(_id) && !ID_MISSING(_id) &&                                    \
   (BKE_idtype_get_info_from_id(id_cast<const ID *>(_id))->flags &             \
...
#define ID_IS_OVERRIDE_LIBRARY_HIERARCHY_ROOT(_id)                             \
  (!ID_IS_OVERRIDE_LIBRARY_REAL(_id) ||                                        \
   (id_cast<ID *>(_id))->override_library->hierarchy_root ==                   \
       (id_cast<ID *>(_id)))

#define ID_IS_ASSET(_id) (id_cast<const ID *>(_id)->asset_data != NULL)

#define ID_TYPE_USE_COPY_ON_EVAL(_id_type)                                     \
  (!ELEM(_id_type, ID_LI, ID_SCR, ID_VF, ID_BR, ID_WM, ID_PAL, ID_PC, ID_WS,   \
         ID_IM))
...
#define ID_TAG_KEEP_ON_UNDO                                                    \
  (ID_TAG_EXTRAUSER | ID_TAG_MISSING | ID_TAG_RUNTIME | ID_TAG_LOCAL |         \
...
enum IDRecalcFlag {
  ...
  ID_RECALC_PSYS_ALL = (ID_RECALC_PSYS_REDO | ID_RECALC_PSYS_RESET |
                        ID_RECALC_PSYS_CHILD | ID_RECALC_PSYS_PHYS),
...
#define FILTER_ID_ALL                                                          \
  (FILTER_ID_AC | FILTER_ID_AR | FILTER_ID_BR | FILTER_ID_CA |                 \
   FILTER_ID_CU_LEGACY | FILTER_ID_GD_LEGACY | FILTER_ID_GR | FILTER_ID_IM |   \
   FILTER_ID_LA | FILTER_ID_LS | FILTER_ID_LT | FILTER_ID_MA | FILTER_ID_MB |  \
   FILTER_ID_MC | FILTER_ID_ME | FILTER_ID_MSK | FILTER_ID_NT | FILTER_ID_OB | \
   FILTER_ID_PA | FILTER_ID_PAL | FILTER_ID_PC | FILTER_ID_SCE |               \
   FILTER_ID_SPK | FILTER_ID_SO | FILTER_ID_TE | FILTER_ID_TXT |               \
   FILTER_ID_VF | FILTER_ID_WO | FILTER_ID_CF | FILTER_ID_WS | FILTER_ID_LP |  \
   FILTER_ID_CV | FILTER_ID_PT | FILTER_ID_VO | FILTER_ID_SIM | FILTER_ID_KE | \
   FILTER_ID_SCR | FILTER_ID_WM | FILTER_ID_LI | FILTER_ID_GP | FILTER_ID_IP)
