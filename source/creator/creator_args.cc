...
void Args::add(const std::string& short_arg,
...
void Args::print_help()
{
    ...
    for (const auto& arg : args)
    {
        ...
        size_t len = arg.short_arg.length() + (arg.short_arg.empty() ? 0 : 2) + arg.long_arg.length();
        if (len < 30)
        {
                ...
                const char** argv)
{
    for (int i = 1; i < argc; ++i)
    {
        ...
        for (const auto& arg : args)
        {
            if ((!arg.short_arg.empty() && std::strcmp(argv[i], arg.short_arg.c_str()) == 0) ||
                (!arg.long_arg.empty() && std::strcmp(argv[i], arg.long_arg.c_str()) == 0))
            {
...
static int arg_handle_version(int,
                              const char**,
                              void*)
{
...
static int arg_handle_background_mode(int,
                                      const char**,
                                      void*)
{
...
static int arg_handle_log_level(int          argc,
                                const char** argv,
                                void*)
{
    if (argc < 2)
...
static int arg_handle_debug(int,
                            const char**,
                            void*)
{
...
void main_args_setup(Args& args)
{
    ...
    args.add("-p", "--window-geometry", "<x> <y> <w> <h> Open with geometry", arg_handle_window_geometry);

    args.add("", "--log-level", "<level> Set logging verbosity", arg_handle_log_level);
    args.add("-d", "--debug", "Turn debugging on", arg_handle_debug);
}

int main_args_handle(int          argc,
                     const char** argv)
{
    Args args;
