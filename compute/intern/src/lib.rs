...
mod intern_ffi {
    ...
    extern "Rust" {
        ...
        fn compute_matrix_vector_muls_rs(matrices: &[f32], vectors: &[f32], outs: &mut [f32], count: usize);
...
pub fn compute_matrix_vector_muls_rs(matrices: &[f32], vectors: &[f32], outs: &mut [f32], count: usize) {
    unsafe {
        math_accel::vk_compute_matrix_vector_muls(matrices.as_ptr(), vectors.as_ptr(), outs.as_mut_ptr(), count);
    }
}
